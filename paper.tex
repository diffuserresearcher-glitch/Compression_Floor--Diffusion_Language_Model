...
\begin{abstract}
Block-diffusion language models accelerate generation by predicting multiple tokens per denoising step, but the limits of this compression remain poorly understood. Therefore, we study SDAR-4B-Chat (a 4B-parameter block-diffusion language model) with block size \textbf{32} on a mathematical benchmark and identify a sharp \textbf{compression floor}, the limit beyond which predicting more tokens per step causes generation quality to collapse, with three properties. First, generation quality does not gradually decline as more tokens are predicted per step; instead, outputs tend to remain stable or suddenly collapse into repetition: across pooled schedules, \textbf{60\%} of outputs remain largely repetition-free (repeat-4 < \textbf{0.2}), while \textbf{23\%} collapse into severe repetition loops (repeat-4 > \textbf{0.7}), with only \textbf{9\%} in the intermediate regime (repeat-4 between \textbf{0.3} and \textbf{0.6}). Second, supervised fine-tuning (SFT) can worsen compression tolerance. Dense supervision on all masked positions ($L_{\mathrm{all}}$) reduces one-token-per-step accuracy from \textbf{48\%} to \textbf{8\%}, whereas supervision on commit-only positions ($L_{\mathrm{commit}}$; positions selected for final prediction) holds it at \textbf{20\%}. SFT also reduces the usable compression range from four to two tokens per step. Third, on-policy self-distillation inherits this floor through trajectory yield: at temperature \textbf{0.9}, the student produced \textbf{0} usable trajectories in \textbf{28} attempts, while temperature \textbf{0.3} with a larger generation budget yielded \textbf{38/50}. We replicate this yield collapse on a second model family, suggesting the phenomenon is not architecture-specific. Together, these results show that few-step diffusion language modeling is governed not only by the number of tokens revealed per step, but also by how supervision and sampling interact with the model’s compression floor. Our findings highlight supervision density and sampling strategy as two practical levers for making accelerated diffusion-LM training reliable. The code is available at \url{https://anonymous.4open.science/r/Compression_Floor--Diffusion_Language_Model-1BC2/README.md}.
...
Our results reveal that the \textit{compression floor} is not a smooth degradation that training can simply polish away. Instead, aggressive decoding produces a \textit{sharp, bimodal failure mode}: generations tend either to remain largely coherent or to collapse into severe repetition loops. Moreover, SFT does not widen the usable decoding range; every variant we train narrows it, with the extent of degradation strongly dependent on \textit{supervision density}. These findings suggest that the achievable speedup of block-diffusion models is constrained by an intrinsic interaction between \textit{decoding compression} and \textit{the way the model is trained}.
...
Recent work has pursued \textit{few-step diffusion language modeling} along two complementary directions. Both masked and block-diffusion hybrids such as SDAR~\citep{cheng2025sdar} and TraDo~\citep{wang2025tracerl} reduce sequential depth by revealing multiple tokens in parallel, while training-free methods such as Fast-dLLM~\citep{wu2025fastdllm} further accelerate inference without modifying the model. These approaches expose a fundamental trade-off between \textit{generation speed and the number of denoising steps}, and recent analyses attribute few-step degradation to errors introduced when multiple tokens must be predicted simultaneously.
...
Our results have several limitations. We use a single 40-problem benchmark and one random seed, giving accuracy estimates with binomial uncertainty of about $\pm15$ points; the results should therefore be viewed as evidence of large effects rather than precise estimates. All fine-tuning experiments use LoRA on a single 4B model, so behavior may differ with full fine-tuning or larger models. The dense and commit-only training arms also differ in data reuse and target cleanliness, preventing us from attributing their performance gap solely to the training objective. In particular, most reference solutions exceed the training length limit, producing truncated targets without stop tokens and potentially contributing to the non-termination observed across fine-tuned checkpoints. Finally, we do not measure the final performance after self-distillation: although 38 trajectories were accepted, privileged-teacher distillation rapidly drove the loss near zero, suggesting that the teacher provided little additional information beyond the student's distribution. Our results therefore establish the conditions needed for distillation, but not its eventual benefit.

Overall, we find that the compression limit of a small block-diffusion language model is not a smooth decline in performance. Instead, individual samples undergo a sharp transition from successful generation to repetitive collapse. Fine-tuning worsens this limit across all variants tested, with supervision density playing an important role: commit-only supervision reduces the degradation relative to dense supervision but does not eliminate it. We further find that self-distillation depends critically on obtaining a sufficient yield of usable trajectories, which is strongly affected by sampling temperature and generation budget. Future work should therefore report per-sample collapse rates, supervision density, and trajectory yield alongside final distillation performance.
